...
class FlowStep(nn.Module):
    def __init__(self, level, s, channel_dim, input_shape, filter_size,
        ...
        self.conditionalCoupling = modules.ConditionalCoupling(
                                    ...
                                    cond_channels, filter_size, noscale,
                                    noscaletest)
    ...
    def forward(self, z, lr_feat_map=None, x_lr=None, logdet=0, reverse=False):
        ...
        if not reverse:
            ...
            z, logdet = self.conditionalCoupling(z, lr_feat_map=lr_feat_map, logdet=logdet, reverse=False)

            return z, logdet

        else:
            # 1. Conditional Coupling
            z, logdet = self.conditionalCoupling(z, lr_feat_map=lr_feat_map, logdet=logdet, reverse=True)
            # 2. Invertible 1x1 convolution
...
class NormFlowNet(nn.Module):
    def __init__(self, input_shape, filter_size, bsz, s,
        ...
        for i in range(self.L):
            self.downsample_convs.append(nn.Conv2d(cond_channels, cond_channels,
        ...
        for i in range(self.L):
            ...
            if i < L - 1:
                ...
                self.level_modules[i].append(modules.GaussianPrior(C, s, cond_channels, (bsz, C, H, W)))
                C = C // 2
                self.output_shapes.append((-1, C, H, W))
            ...
            for i in range(self.L):
                for layer in self.level_modules[i]:
                    if isinstance(layer, modules.Squeeze):
                        ...
                        z, logdet, logpz = layer(z, logdet=logdet,
                                                 logpz=logpz, lr_feat_map=lr_downsampled_feats[i + 1],
            ...
            for i in reversed(range(self.L)):
                for layer in reversed(self.level_modules[i]):
                    if isinstance(layer, modules.GaussianPrior):
                        z, logdet, logpz = layer(z, lr_feat_map=lr_downsampled_feats[i + 1],
                                                 logdet=logdet, logpz=logpz, eps=eps,
                                                 reverse=True, use_stored=use_stored)

                    elif isinstance(layer, FlowStep):
                        z, logdet = layer(z=z, lr_feat_map=lr_downsampled_feats[i + 1],
