...
from models.transformations import baseline_stflow_modules as modules
...
class FlowStep(nn.Module):
    ...
    def forward(self, z, h=None, x_lr=None, logdet=0, reverse=False):
        ...
        if not reverse:
            ...
            z = z.unsqueeze(2)
            z, logdet = self.conditionalCoupling(z=z,
            ...
            z, logdet = self.conditionalCoupling(z,
...
class NormFlowNet(nn.Module):
        ...
        for i in range(self.L):
            ...
            for k in range(K):
                self.level_modules[i].append(FlowStep(i, s, C_modules, input_shape,
                                             filter_size, cond_channels,
                                             noscale, noscaletest, testmode)).to('cuda')

            if i < L - 1:
                # 3.Split Prior for intermediate latent variables
                self.level_modules[i].append(modules.GaussianPrior(C_modules, s, i, cond_channels, (bsz, C_modules, lag_len, H, W)))
                # C_prior = C // 2
                # self.output_shapes.append((bsz, C, lag_len, H, W))

        self.level_modules[-1].append(modules.GaussianPrior(C_modules, s, i, cond_channels,
    ...
    def forward(self, x=None, x_past=None, state=None, logdet=0,
                ...
                for layer in self.level_modules[i]:
                    ...
                    if isinstance(layer, modules.SqueezeTS):
                        ...
                        for s in range(self.lag_len):
                            z_shaped[:,:,0,:,:] = layer(x[:,:,0,:,:], reverse=False)

                        z = z_shaped

                    elif isinstance(layer, FlowStep):
                        # TODO: could adapt by just conditioning on previous latents not h
                        z, logdet = layer(z,
                                          logdet=logdet, reverse=False)

                    elif isinstance(layer, modules.GaussianPrior):
                        # TODO: adapt to baseline prior
        ...
        # Decode
            ...
            for i in range(self.L):
                ...
                h_temp = torch.zeros((1,c,t,height,w)).to(self.device).cuda()
            ...
            for i in reversed(range(self.L)):
                for layer in reversed(self.level_modules[i]):
                    ...
                    if isinstance(layer, modules.GaussianPrior):
                        ...
                        h_temp = torch.zeros((1,c,t,height,w)).to(self.device)
...
class FlowModel(nn.Module):
    ...
    def forward(self, x=None, x_past=None, z=None, logdet=0, state=None,
        ...
        if not reverse:
            pdb.set_trace()
            out = self.normalizing_flow(x=x, x_past=x_past, state=state)
            return out 
    ...
    def normalizing_flow(self, x, x_past, state=None):
        ...
        # z, logdet = self._dequantize_uniform(x, self.nbins)
        logdet = torch.zeros_like(x[:, 0, 0, 0, 0])
        # z, logdet = self.sigmoid(x[0, 0, 0, ...], logdet, reverse=True)
        # pdb.set_trace()
        # z = z.unsqueeze(0).unsqueeze(0).unsqueeze(0)

        # logdet = logdet + float(-np.log(self.nbins) * np.prod(x.size()[1:]))
        # logdet = logdet + float(-np.log(74.694879) * np.prod(x.size()[1:]))

        # z = x
        # z, logdet = self._variational_dequantizer(z[:,0,:,:,:], logdet)
        # z = z.unsqueeze(1) # TODO: need to change later on when including longer lag length as forecasting window

        # Push z through flow
        z, state, logdet, logp_z = self.flow.forward(x=x, x_past=x_past,
                                                     logdet=logdet,
                                                     state=state)
